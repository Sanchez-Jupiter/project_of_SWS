...
function able_to_move(dire, posi, barri, obj){
    const x = posi[0];
    const y = posi[1];
    for(let i = 0; i < array_length(barri); i = i + 1){
        const xb = query_position(barri[i])[0];
        const yb = query_position(barri[i])[1];
        if(dire === LEFT && x > xb && math_abs(x - xb) < CLOSEST_DIS){
            return false;
        }
        else if(dire === RIGHT && x < xb && math_abs(x - xb) < CLOSEST_DIS){
            return false;
        }
        else if(dire === UP && y < yb && math_abs(y - yb) < CLOSEST_DIS){
            return false;
        }
        else if(dire === DOWN && y > yb && math_abs(y - yb) < CLOSEST_DIS){
            return false;
        }
    }
    return true;
...
update_loop(game_state => {
    ...
    if(gameobjects_overlap(player[cond], player2[cond2])){
        ...
        && math_abs(query_position(player[cond])[1] - query_position(player2[cond2])[1]) < 30
            ...
            update_position(player[cond], [query_position(player[cond])[0] - punched_dis, query_position(player[cond])[1]]);
            ...
            && math_abs(query_position(player[cond])[1] - query_position(player2[cond2])[1]) < 30
                ...
                update_position(player[cond], [query_position(player[cond])[0] + punched_dis, query_position(player[cond])[1]]);
                if(cond !== punch_left && cond !== punch2_left){
                    hp1 = hp1 - PUNCHED_DEHP;
                }
    ...
    /*
    const res_of_check_barri = check_barri(player[cond], barri);
    if(res_of_check_barri !== -1){
        if(query_position(player[cond])[1] < query_position(barri[res_of_check_barri])[1]){
            g = 0;
   ...
   if (input_key_down("j") ) {
       if(able_to_move(UP, addtion_of_vec(query_position(player[cond], [0, JUMP_HEIGHT])), barri, player[cond])){
       ...
       if(able_to_move(LEFT, addtion_of_vec(query_position(player[cond]),[-1 * movement_dist, 0]) , barri, player[cond])){
           update_position(player[cond], [-inf, -inf]);
       ...
       if(able_to_move(RIGHT, addtion_of_vec(query_position(player[cond]), [movement_dist, 0]), barri, player[cond])){
                update_position(player[cond], [-inf, -inf]);
   ...
   else if(input_key_down("k")){
       
       if(cond === player_towards_right2 || cond === player_towards_right || cond === punch_right || cond === punch2_right){
           //cond = punch_right;
           if(able_to_move(RIGHT, addtion_of_vec(query_position(player[cond], barri),[movement_dist, 0]), player[cond])){
                update_position(player[cond], [-inf, -inf]);
                if(cond === punch_right){
                    cond = punch2_right;
                }
   ...
   if(query_position(player[cond])[0] > 0 && query_position(player[cond])[0] < 550 
        && query_position(player[cond])[1] > 0 && query_position(player[cond])[1] < 550){
            if(able_to_move(DOWN, [query_position(player[cond])[0], query_position(player[cond])[1] + g], barri, player[cond])){
    ...
    if(gameobjects_overlap(player[cond], player2[cond2])){
        ...
        && math_abs(query_position(player[cond])[1] - query_position(player2[cond2])[1]) < 30
            ...
            update_position(player2[cond2], [query_position(player2[cond2])[0] - punched_dis, query_position(player2[cond2])[1]]);
            ...
            && math_abs(query_position(player[cond])[1] - query_position(player2[cond2])[1]) < 30
                ...
                update_position(player2[cond2], [query_position(player2[cond2])[0] + punched_dis, query_position(player2[cond2])[1]]);
                if(cond2 !== punch_left && cond2 !== punch2_left){
                    hp2 = hp2 - PUNCHED_DEHP;
                }
        }
    }
    ...
    /*
    const res_of_check_barri2 = check_barri(player2[cond2], barri);
    if(res_of_check_barri2 !== -1){
        if(query_position(player[cond])[1] < query_position(barri[res_of_check_barri])[1]){
            g = 0;
            update_position(player2[cond2], [new_position2[0], new_position2[1] - BARRI_BOUNCE_DIS]);
        }
        else if( last_dir2 === UP ){
            update_position(player2[cond2], [new_position2[0], new_position2[1] + BARRI_BOUNCE_DIS]);
        }
   ...
   if (input_key_down("1") ) {
       if(able_to_move(UP, query_position(player2[cond2]), barri, player[cond])){
   ...
   if (input_key_down("n")) {
       if(able_to_move(LEFT, query_position(player2[cond2]), barri, player[cond])){
           update_position(player2[cond2], [-inf, -inf]);
   ...
   if (input_key_down("m")) {
       if(able_to_move(RIGHT, query_position(player2[cond2]), barri, player[cond])){
                update_position(player2[cond2], [-inf, -inf]);
   ...
   if(query_position(player2[cond2])[0] > 0 && query_position(player2[cond2])[0] < 550 
        && query_position(player2[cond2])[1] > 0 && query_position(player2[cond2])[1] < 550){
            if(able_to_move(DOWN, query_position(player2[cond2]), barri, player[cond])){
