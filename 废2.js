...
const LEGNTH_OF_PLAYER = 70;
const WIDTH_OF_PLAYER = 35;
...
const barri_height = [50, 50];
const barri_length = [200, 200];
...
function is_over(obj, barri){
    for(let i = 0; i < array_length(barri); i = i + 1){
        if( gameobjects_overlap(barri[i], obj) 
        && math_abs(query_position(obj)[0] - query_position(barri[i])[0]) <= math_abs(WIDTH_OF_PLAYER - barri_length) / 2
        && query_position(obj)[1] < query_position(barri[i])[1]
        ){
            return true;
        }
    }
...
function is_left(obj, barri){
    for(let i = 0; i < array_length(barri); i = i + 1){
        if( gameobjects_overlap(barri[i], obj) 
        && math_abs(query_position(obj)[1] - query_position(barri[i])[1]) <= math_abs(LEGNTH_OF_PLAYER - barri_height) / 2
        && query_position(obj)[0] < query_position(barri[i])[0]
        ){
            return true;
        }
    }
...
update_loop(game_state => {
    ...
    for(let i = 0; i < array_length(barri); i = i + 1){
        if(is_under(player[cond], [barri[i]])){
            while(gameobjects_overlap(barri[i], player[cond])){
                update_position(player[cond], [query_position(player[cond])[0], query_position(player[cond])[1] + 1]);
            }
        }
        else if(is_over(player[cond], [barri[i]])){
            while(gameobjects_overlap(barri[i], player[cond])){
                update_position(player[cond], [query_position(player[cond])[0], query_position(player[cond])[1] - 1]);
            }
        }
        else if(is_left(player[cond], [barri[i]])){
            while(gameobjects_overlap(barri[i], player[cond])){
                update_position(player[cond], [query_position(player[cond])[0] - 1, query_position(player[cond])[1]]);
            }
        }
        else if(is_right(player[cond], [barri[i]])){
            while(gameobjects_overlap(barri[i], player[cond])){
                update_position(player[cond], [query_position(player[cond])[0] + 1, query_position(player[cond])[1]]);
            }
        }
   ...
   if (input_key_down("j") ) {
       ...
       if( check_barri(player[cond], barri) && is_under(player[cond], barri)){
