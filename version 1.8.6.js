...
const sword = update_scale( create_sprite("https://raw.githubusercontent.com/Sanchez-Jupiter/project_of_SWS/main/sword.jpg"), [0.07, 0.07]);
...
function is_towards_right(cond){
    if(cond === player_towards_right2 || cond === player_towards_right
        ...
        || cond === wave_rigth ||  cond === sword_right
...
update_loop(game_state => {
    ...
    if(gameobjects_overlap(player[cond], player2[cond2])){
        ...
        && math_abs(query_position(player[cond])[1] - query_position(player2[cond2])[1]) < 30
            ...
            const y = query_position(player[cond])[1];
            if(cond !== punch_right && cond !== punch2_right && cond !== defense_right){
            ...
            && math_abs(query_position(player[cond])[1] - query_position(player2[cond2])[1]) < 30
                ...
                const x = query_position(player[cond])[0] - punched_dis;
                const y = query_position(player[cond])[1];
                if(cond !== punch_left && cond !== punch2_left && cond !== defense_left){
                    hp1 = hp1 - PUNCHED_DEHP;
                }
                update_position(player[cond], [-inf, -inf]);
                cond = attacked_right;
    ...
    //----------------after punch, no need to be upgraded-----------------------
   ...
   if (input_key_down("t") && cond !== defense_right && cond !== defense_left) {
   ...
   if (input_key_down("q") && cond !== defense_right && cond !== defense_left) {
        if(check_barri(player[cond], barri)){
   ...
   if (input_key_down("w") && cond !== defense_right && cond !== defense_left) {
       if(check_barri(player[cond], barri)){
   ...
   if(input_key_down("y") && cond !== defense_right && cond !== defense_left){
