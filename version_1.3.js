...
function check_barri(player, barri){
    ...
    for(let j = 0; j < array_length(boundry); j = j + 1){
        if( gameobjects_overlap(boundry[j], player) ){
            return true;
        }
    }
    return false;
...
update_loop(game_state => {
    ...
    if( check_barri(player[cond], barri) ){
        if( last_dir === UP ){
            update_position(player[cond], [new_position[0], new_position[1] + 1 * movement_dist]);
        }
        else if(last_dir === DOWN ){
            update_position(player[cond], [new_position[0], new_position[1] - 1 * movement_dist]);
        }
        else if(last_dir === LEFT ){
            update_position(player[cond], [new_position[0] + 1 * movement_dist, new_position[1]]);
        }
    ...
    */
    ...
    if(posi[0] < 0 ) {
        update_position(player[cond], [0, posi[1]]);
    }
    if(posi[0] > 550){
        update_position(player[cond], [550, posi[1]]);
    }
    if(posi[1] < 0){
        update_position(player[cond], [posi[0], 0]);
    }
    if(posi[1] > 550){
   ...
   if (input_key_down("w")) {
      // update_position( player, [-inf, -inf] );
       ...
       if( check_barri(player[cond], barri) ){
            ...
            if(last_dir === UP) {
                add_vectors(new_position, [0, 3 * movement_dist]);
                update_position(player[cond], new_position);
            }
       }
       last_dir = UP;
   ...
   if (input_key_down("a")) {
        ...
        if( gameobjects_overlap(player[cond], barri[0]) ){
            //display(cond);
            if(last_dir === LEFT){
                add_vectors(new_position, [3 * movement_dist, 0]);
                update_position(player[cond], new_position);
            }
       }
       else {
    ...
    if (input_key_down("s")) {
       ...
       if( check_barri(player[cond], barri) ){
           if(last_dir === DOWN){
                add_vectors(new_position, [0, -1 * 3 * movement_dist]);
                update_position(player[cond], new_position);
           }
   ...
   if (input_key_down("d")) {
       if( check_barri(player[cond], barri) ){
           if(last_dir === RIGHT){
                add_vectors(new_position, [-1 * 3 * movement_dist, 0]);
                update_position(player[cond], new_position);
           }
       }
       else{
